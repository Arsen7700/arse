...
| `TELEGRAM_BOT_TOKEN` | Render backend | Секретный токен бота от `@BotFather`; только в Environment Render, никогда во frontend/GitHub |
| `TELEGRAM_CHAT_ID` | Render backend | ID личного чата или группы, куда бот отправляет отчёты |
| `TELEGRAM_ADMIN_KEY` | Render backend | Случайный длинный секрет для ручной отправки и изменения расписания; вводится в окне отчёта, не сохраняется браузером |
...
В окне «Отчёт для Telegram» есть оба способа отправки: вручную отправить отчёт за выбранную дату или включить ежедневную отправку. Для отправки нужен бот Telegram и секреты в Environment backend на Render:

1. Создайте бота через `@BotFather` и скопируйте токен. Не отправляйте токен в чат или в GitHub.
2. Откройте личный чат с ботом, нажмите Start или отправьте `/start`. Иначе Telegram не разрешит боту начать личную переписку.
3. Получите ID получателя безопасным способом (например, через собственного бота, который возвращает `message.chat.id`) и задайте на Render `TELEGRAM_CHAT_ID`.
4. На Render задайте `TELEGRAM_BOT_TOKEN`, `TELEGRAM_CHAT_ID` и `TELEGRAM_ADMIN_KEY`. Для ключа администратора создайте отдельный длинный случайный секрет. После изменения переменных перезапустите/redeploy backend.
5. Откройте «Продажи → Отчёт для Telegram», введите `TELEGRAM_ADMIN_KEY`, выберите период «За день» или «За месяц», задайте дату/месяц и нажмите «Отправить отчёт сейчас». Отчёт содержит продажи по каждому товару и его сумму за этот период. Для повтора не нужен повторный запуск приложения.
6. Чтобы включить расписание, отметьте ежедневную отправку, задайте время и часовой пояс и нажмите «Сохранить расписание». Настройки сохраняются в базе, а последний успешный день отправки не дублируется при обычном перезапуске.

Время расписания считается в выбранном часовом поясе. Текущая реализация проверяет расписание работающим процессом API примерно раз в 30 секунд. Поэтому она не гарантирует отправку в точную минуту, если Render останавливает или перезапускает сервис: при пробуждении пропущенный за сегодня отчёт отправится позднее. Render Free может переводить сервис в сон. Для надёжной доставки в заданное время нужен always-on backend/worker либо отдельный Render Cron Job; один API-процесс должен быть активен, чтобы выполнять проверку. Не запускайте несколько копий backend без настройки координации, иначе они могут одновременно отправить один отчёт.

Если адрес сайта открыт всем, любой человек может видеть форму отчёта, но отправить сообщение или изменить расписание сможет только знающий `TELEGRAM_ADMIN_KEY`. Не используйте этот ключ повторно для других сервисов. Если ключ скомпрометирован — замените его в Render и перезапустите backend.
